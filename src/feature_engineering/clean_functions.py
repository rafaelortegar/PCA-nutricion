...
def convierte_a_numericas(df):
    """
    Función que convierte las respuestas de las preguntas en una codificación del 1 al 10.
    """
    for i in range(1, 155):
        columna = 'p' + str(i)
        df.loc[df[columna] == "A", columna] = 1
        df.loc[df[columna] == "B", columna] = 2
        df.loc[df[columna] == "C", columna] = 3
        df.loc[df[columna] == "D", columna] = 4
        df.loc[df[columna] == "E", columna] = 5
        df.loc[df[columna] == "F", columna] = 6
        df.loc[df[columna] == "G", columna] = 7
        df.loc[df[columna] == "H", columna] = 8
        df.loc[df[columna] == "I", columna] = 9
        df.loc[df[columna] == "J", columna] = 10
...
def prepara_datos():
    ...
    print("\t-> Convertimos las respuestas del cuestionario de string a numéricas")
    dataset_unido = convierte_a_numericas(dataset_unido)
